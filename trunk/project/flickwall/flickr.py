...
API_KEY = '2028a9e776f8660de1b5aa4b848371d2'
API_SECRET = '77eac96882ab8a88'
email = ""
password = ""
AUTH = False
...
class Photo(object):
	...
	def getExif(self):
		...
		for exif in data.rsp.photo.exif:
			...
			ret.append({exif.tag:exif.raw.text})
		return ret
...
def _doget(method, auth=False, **params):
	...
	return _get_data(minidom.parse(urlopen(url,proxies=PROXY_INFO)))
...
def _dopost(method, auth=False, **params):
	...
	return _get_data(minidom.parse(urlopen(url, payload,proxies=PROXY_INFO)))
